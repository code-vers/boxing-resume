...
import { AddMatchDrawer } from "./AddMatchDrawer";
...
interface MatchRecordFiltersProps {
...
/**
 * @component MatchRecordFilters
 * @description Action strip and filter controls for the Match Records page.
 */
export function MatchRecordFilters({
  values,
  divisions,
  onChange,
}: MatchRecordFiltersProps) {
  const [isAddMatchOpen, setIsAddMatchOpen] = useState(false);

  const updateFilters = (nextValues: Partial<MatchFilterValues>) => {
      ...
      <div className='flex min-h-16 w-full flex-col items-stretch gap-2.25 rounded-[5px] border border-[#e3ded4] bg-white px-4 py-3.5 shadow-[0_1px_2px_rgba(0,0,0,0.04)] lg:flex-row lg:items-center lg:gap-2'>
        <div className='relative min-w-0 flex-1 lg:max-w-175'>
          ...
          <input
            ...
            placeholder='Search fighter name, event, title...'
          />
        </div>
